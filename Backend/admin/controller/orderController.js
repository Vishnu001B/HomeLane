...
exports.getTotalProductQuantityByState = async (req, res) => {
  try {
      const { state, vendorId } = req.params; // Assuming vendorId is also passed as a parameter

      if (!state || !vendorId) {
          return res.status(400).json({ message: "State and vendorId are required" });
      }
      ...
      const result = await recivedOrder.aggregate([
          {
              $match: {
                  'addresses.state': state, // Match orders where any address has the specified state
                  vendorId: vendorId // Match orders where vendorId matches
              }
          },
          {
          ...
          {
              $group: {
                  _id: '$addresses.state', // Group by state
